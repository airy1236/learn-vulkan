...
struct Vertex {
	...
	static std::array<VkVertexInputAttributeDescription, 3> getAttributeDescriptions() {
		...
		//添加一个新的vec2类型变量来存储纹理坐标
		//添加一个新的VkVertexInputAttributeDescription对象用于在顶点着色器访问顶点的纹理坐标,这样才能将纹理坐标传递给片段着色器使用
...
private:
	...
	VkImageView textureImageView; //存储纹理图像的图像视图对象
	...
	void createLogicalDevice() {
		...
		//需要在创建逻辑设备时检查这一设备特性是否被设备支持才能使用各向异性过滤
	...
	void createDescriptorSetLayout() {
		...
		//在createDescriptorSetLayout函数中填写用于组合图像采样器描述符的VkDescriptorSetLayoutBinding结构体信息
		...
		//设置stageFlags成员变量指明在片段着色器中使用组合图像采样器描述符。
		//在顶点着色器也可以进行纹理采样，一个常见的用途是在顶点着色器中使用高度图纹理来对顶点进行变形
	...
	void createTextureSampler() {
		...
		//指定纹理需要放大和缩小时使用的插值方法
		//纹理放大会出现采样过密的问题，纹理缩小会出现采样过疏的问题
		//对于这两个成员变量，设置其值为VK_FILTER_NEAREST或VK_FILTER_LINEAR，分别对应 邻近过滤 和 线性插值两种过滤方式
		samplerInfo.magFilter = VK_FILTER_LINEAR;
		samplerInfo.minFilter = VK_FILTER_LINEAR;

		//指定寻址模式
		//这里的U、V、W对应X、Y和Z轴。它们的值可以是下面这些：
		...
		//通常来说，只要性能允许，都会默认开启各向异性过滤。
		//maxAnisotropy成员变量用于限定计算最终颜色使用的样本个数，值越小，采样的性能表现越好，但采样结果质量较低。
		//目前为止，还没有图形硬件能够使用超过16个样本，同时即使可以使用超过16个样本，采样效果的增强也开始变得微乎其微
		...
		//边界颜色并非可以设置为任意颜色。它可以被设置为浮点或整型格式的黑色、白色或透明色。
		samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK;


		samplerInfo.unnormalizedCoordinates = VK_FALSE; //指定采样使用的坐标系统
		//将其设置为VK_TRUE时，采样使用的坐标范围为[0, texWidth)和[0, texHeight)。
		//将其设置为VK_FALSE，采样使用的坐标范围在所有轴都是[0, 1)。
		//通常使用VK_FALSE的情况更常见，这种情况下可以使用相同的纹理坐标采样不同分辨率的纹理。

		//通过compareEnable和compareOp成员变量，可以将样本和一个设定的值进行比较，然后将比较结果用于之后的过滤操作。
		//通常在进行阴影贴图时会使用它。暂时不需要使用这一功能
		samplerInfo.compareEnable = VK_FALSE;
		samplerInfo.compareOp = VK_COMPARE_OP_ALWAYS;
		
		//mipmapMode、mipLodBias、minLod和maxLod成员变量用于设置分级细化(mipmap)，它可以看作是过滤操作的一种
	...
	void createDescriptorPool() {
		//修改createDescriptorPool函数，添加一个用于组合图像采样器描述符的VkDescriptorPoolSize结构体信息
	...
	VkImageView createImageView(VkImage image, VkFormat format) {
		...
		//由于VK_COMPONENT_SWIZZLE_IDENTITY的值实际上是0，所以我们可以省略对viewInfo.components的显式初始化，
		//将viewInfo.components的成员变量都设置为0。最后，调用vkCreateImageView函数创建图像视图对象：
