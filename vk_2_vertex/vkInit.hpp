...
private:
	...
	void createVertexBuffer() {
		...
		bufferInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; //缓冲中的数据的使用目的,可以使用位或运算来指定多个使用目的
		bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; //缓冲可以被特定的队列族所拥有，也可以同时在多个队列族之前共享
		//flags成员变量用于配置缓冲的内存稀疏程度，将其设置为0使用默认值
		...
		vkBindBufferMemory(device, vertexBuffer, vertexBufferMemory, 0/*偏移值*/ );

		//填充顶点缓冲
		void* data;
		vkMapMemory(device, vertexBufferMemory, 0, bufferInfo.size, 0, &data); //通过给定的 内存偏移值 和 内存大小 访问特定的内存资源
	...
	//需要结合自己的需求选择最合适的内存类型使用
	//添加一个叫做findMemoryType的函数来做件事：
	uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
		...
		for (uint32_t i = 0; i < memProperties.memoryTypeCount; ++i) {
			...
			//typeFilter指定需要的内存类型的位域，遍历可用内存类型数组，检测每个内存类型是否满足我们需要即可(相应位域为1)
			//需要位域满足VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT(用于从CPU写入数据)和
			//VK_MEMORY_PROPERTY_HOST_COHERENT_BIT(后面介绍原因)的内存类型
		}
