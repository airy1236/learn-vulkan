...
public:
	...
	void drawFrame(Camera camera/*加入相机组件*/) {
...
private:
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
		{
			...
			-> UINT32*/); //由于使用的模型包含的顶点个数远远大于65535，所以不能使用uint16_t作为索引的数据类型，而应该使用uint32_t作为索引的数据类型
	...
	void updateUniformBuffer(uint32_t currentImage, Camera camera/*添加相机组件*/) {
		...
		ubo.projection[1][1] *= -1; //vulkan的Y轴与OpenGL是相反的
