...
private:
	...
	void createVertexBuffer() {
		...
		//使用新的关联stagingBufferMemory作为内存的stagingBuffer缓冲对象来存放CPU加载的顶点数据
		//使用下面两个缓冲使用标记：
		...
		//只能通过stagingBuffer来向vertexBuffer复制数据，需要使用标记指明使用缓冲进行传输操作
		
		//在createVertexBuffer函数中调用copyBuffer函数复制顶点数据到显卡读取较快的缓冲中
	...
	void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
		...
		//使用VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT标记告诉驱动程序我们如何使用这个指令缓冲，来让驱动程序进行更好的优化

		vkBeginCommandBuffer(commandBuffer, &beginInfo);

		//VkBufferCopy数组指定了复制操作的源缓冲位置偏移，目的缓冲位置偏移，以及要复制的数据长度
		VkBufferCopy copyRegion{}; 
		copyRegion.srcOffset = 0; // Optional
		copyRegion.dstOffset = 0; // Optional
		copyRegion.size = size;
		//使用vkCmdCopyBuffer指令来进行缓冲的复制操作
		...
		//等待传输操作完成，有两种等待内存传输操作完成的方法：
		// 一种是使用栅栏(fence)，通过vkWaitForFences函数等待。
		// 另一种是通过vkQueueWaitIdle函数等待。
