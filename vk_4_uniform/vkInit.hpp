...
public:
	...
	void drawFrame() {
		...
		//调用updateUniformBuffer函数可以在每一帧产生一个新的变换矩阵
...
private:
	...
	//所有的描述符绑定被组合进一个VkDescriptorSetLayout对象
	//在pipelineLayout成员变量的定义上面定义descriptorSetLayout成员变量
	...
	//在管线创建时提供着色器使用的每一个描述符绑定信息
	void createDescriptorSetLayout() {
		VkDescriptorSetLayoutBinding uboLayoutBinding = {};
		//binding和descriptorType成员变量用于指定着色器使用的描述符绑定和描述符类型
		uboLayoutBinding.binding = 0;
		uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; //描述类型指定为uniform buffer类型
		uboLayoutBinding.descriptorCount = 1; //descriptorCount成员变量用来指定数组中元素的个数
										      //可以使用数组来指定骨骼动画使用的所有变换矩阵
											  //MVP矩阵只需要一个uniform缓冲对象，所以将descriptorCount的值设置为1
		uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; //指定描述符在哪一个着色器阶段被使用
		//stageFlags成员变量可以指定通过VkShaderStageFlagBits组合或VK_SHADER_STAGE_ALL_GRAPHICS指定描述符被使用的着色器阶段
		uboLayoutBinding.pImmutableSamplers = nullptr; // Optional //pImmutableSamplers成员变量仅用于和图像采样相关的描述符
		
		//调用vkCreateDescriptorSetLayout函数创建VkDescriptorSetLayout对象
		//vkCreateDescriptorSetLayout函数以VkDescriptorSetLayoutCreateInfo结构体作为参数
	...
	void createGraphicsPipeline() {
		...
		//修改VkPipelineRasterizationStateCreateInfo结构体的frontFace成员变量对于正面的设置，
		//来解决设置的投影矩阵将Y轴反转，导致顶点按照顺时针绘制构成背面，从而被背面剔除
		...
		//需要在管线创建时指定着色器需要使用的描述符集布局，管线布局对象指定了管线使用的描述符集布局
		//修改VkPipelineLayoutCreateInfo结构体信息引用布局对象
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		//修改createCommandBuffers函数为每个交换链图像绑定对应的描述符集
		//和顶点缓冲、索引缓冲不同，描述符集并不是图形管线所独有的，所以需要指定我们要绑定的是图形管线还是计算管线
		//管线类型之后的参数是描述符所使用的布局
		//后面三个参数用于指定描述符集的
		//第一个元素索引，需要绑定的描述符集个数，以及用于绑定的描述符集数组
		//最后两个参数用于指定动态描述符的数组偏移
	...
	//updateUniformBuffer函数可以在每一帧产生一个新的变换矩阵
